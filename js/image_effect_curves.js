        ...
        if (node.comfyClass === "EsesImageEffectCurves") {
            ...
            function setupCustomWidgets() {
                ...
                resetButton.callback = () => {
                    if (node.resetButtonState === 'armed') {
                        ...
                        if (node.activeChannel === 'rgb') node.allCurves = {
                            rgb: defaultCurve(),
                            r: defaultCurve(),
                            g: defaultCurve(),
                            b: defaultCurve(),
            ...
            Object.assign(node, {
                ...
                sendCurveUpdateToPython: async function () {
                    ...
                    try {
                        const result = await api.fetchApi("/eses_channel_curves/apply", {
                            method: "POST", headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({ base_data_b64, all_curves_json: curvesWidget.value, base_data_type })
                        }).then(r => r.json());
                ...
                onDrawForeground: function (ctx) {
                    ...
                    if (this.imagePreview) {
                        ...
                        const channelColors = {
                            rgb: "rgba(255,255,0,1)", r: "rgba(255,100,100,1)", g: "rgba(100,255,100,1)", b: "rgba(100,100,255,1)",
                            luma: "rgba(230,230,230,1)", sat: "rgba(255,165,0,1)", mask: "rgba(255,0,255,1)"
                        };

                        const pointColors = {
                            rgb: "rgba(0,255,0,1)", r: "rgba(255,0,0,1)", g: "rgba(0,255,0,1)", b: "rgba(0,0,255,1)",
                            luma: "rgba(230,230,230,1)", sat: "rgba(255,165,0,1)", mask: "rgba(255,0,255,1)"
                        };
                        ...
                        if (activePoints) {
                            const screenPoints = activePoints.map(p => ({
                                x: drawArea.x + p.x * drawArea.width,
                                y: drawArea.y + (1.0 - p.y) * drawArea.height
                            }));
...
api.addEventListener("eses.channel_curves_preview", ({ detail }) => {
    ...
    if (node) {
        ...
        if (totalToLoad === 0 && loadedCount === 0) {
            onAssetLoad();
        }
    }
});